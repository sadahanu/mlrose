...
def gradient_descent(problem, max_attempts=10, max_iters=np.inf,
                     init_state=None, return_statistics=False, has_eval_set=False):
    ...
    while (attempts < max_attempts) and (iters < max_iters):
        ...
        if return_statistics:
            ...
            if has_eval_set:
                fitness_eval_by_iteration[-1].append(problem.fitness_fn.get_val_eval(problem.state))
    ...
    if return_statistics:
        ...
        statistics = {
            ...
            'fitness_eval_by_iteration': [np.array(fbi) for fbi in fitness_eval_by_iteration],
...
class NetworkWeights:
    ...
    def __init__(self, X, y, X_eval, y_eval, node_list, activation, bias=True,
        ...
        if X_eval is not None and y_eval is not None:
            ...
            if len(np.shape(y_eval)) == 1:
                y_eval = np.reshape(y_eval, [len(y_eval), 1])
            assert(y_eval.shape[1] == self.y_true.shape[1])
            assert(X_eval.shape[1] == self.X.shape[1])
    ...
    def get_val_eval(self, state):
        """Evaluate the fitness of a state on the evaluation set.
        ...
        if self.bias:
            ones = np.ones([np.shape(self.X_eval)[0], 1])
            inputs = np.hstack((self.X_eval, ones))
        ...
        for i in range(len(self.weights)):
            ...
            if i < len(self.weights) - 1:
                ...
                y_eval_pred = self.output_activation(outputs)

        # Evaluate loss function
        fitness = self.loss(self.y_eval, y_eval_pred)
        return fitness
...
class NeuralNetwork:
    ...
    def __init__(self, hidden_nodes, activation='relu',
                 ...
                 mutation_prob=0.1, max_attempts=10, restarts=0, random_state=42):
    ...
    def fit(self, X, y, X_eval=None, y_eval=None, init_weights=None):
        ...
        fitness = NetworkWeights(X, y, X_eval, y_eval, node_list, self.activation, self.bias,
        ...
        if self.algorithm == 'random_hill_climb':
            ...
            fitted_weights, loss, statistics = random_hill_climb(
                ...
                restarts=self.restarts, init_state=init_weights, return_statistics=True, has_eval_set=has_eval_set)
            ...
            fitted_weights, loss, statistics = simulated_annealing(
                ...
                return_statistics=True, has_eval_set=has_eval_set)
            self.fit_statistics = statistics
            ...
            if init_weights is None:
                init_weights = np.random.uniform(-1, 1, num_nodes)
            fitted_weights, loss, statistics = gradient_descent(
                problem,
                max_attempts=self.max_attempts, max_iters=self.max_iters,
                init_state=init_weights, return_statistics=True, has_eval_set=has_eval_set)
            self.fit_statistics = statistics
